 ...
 *
 */
class StateViewModelImp<S: State>(
    initialState: S,
    ...
    /**
     * this typically refers to the ViewModel (which should implement StateUpdater)
     */
    override fun initViewModel(viewModel: StateUpdater<S>) {
        vmToDomainObserver = Observer {
            viewState = viewModel.getUpdatedViewState()
            notifyObservers()
        }
    }
    ...
    override fun currentState(): S = viewState
}
